"""
Check saved Ötöslottó tickets against actual draw results.

Compares tickets in tickets_otos with draws in draws_otos where the draw
date has already passed. Reports match count and prize tier for each ticket.

Prize tiers (Ötöslottó):
    5 matches = 1st prize (jackpot)
    4 matches = 2nd prize
    3 matches = 3rd prize
    2 matches = 4th prize
...
PRIZE_TIER = {5: "1st prize (JACKPOT!)", 4: "2nd prize", 3: "3rd prize", 2: "4th prize"}
...
def main():
    ...
    # Check that tickets_otos exists
    exists = conn.execute(
        "SELECT 1 FROM sqlite_master WHERE type='table' AND name='tickets_otos'"
    ).fetchone()
    if not exists:
        print("No tickets found — run otos_generate.py first.")
        conn.close()
        return
    ...
    hits = [r for r in results if r["n"] >= 2]

    print(f"Checked {len(results)} ticket(s).  Prizes found: {len(hits)}\n")
    print(f"{'ID':<5} {'Draw date':<12} {'Ticket':<25} {'Drawn':<25} {'Matches':<20} {'Result'}")
    print("-" * 110)
    ...
    for r in results:
        ...
        print(
            f"{r['id']:<5} {r['draw_date']:<12} "
            f"{str(r['ticket']):<25} {str(r['drawn']):<25} "
            f"{str(r['matches']):<20} {tier_str}{prize_str}"
        )
